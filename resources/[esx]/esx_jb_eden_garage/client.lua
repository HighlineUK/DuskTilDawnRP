...
function OpenMenuGarage(PointType)
	...
	if PointType == 'delete' then
		table.insert(elements,{label = "Return Vehicle", value = 'stock_vehicle'})
	end

	if PointType == 'pound' then
		table.insert(elements,{label = "Return vehicle ("..Config.Price.."$)", value = 'return_vehicle'})
	end
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Go to the Mechanic')
			end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				TriggerEvent('esx:showNotification', 'You cannot store the vehicle')
			end
		end,vehicleProps)
	else
		TriggerEvent('esx:showNotification', 'There is no vehicle to return')
	end
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
		CurrentActionMsg  = "Press ~INPUT_PICKUP~ to get the vehicle"
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
		CurrentActionMsg  = "Press ~INPUT_PICKUP~ to return the vehicle"
		CurrentActionData = {}
	end
